...
export async function GET(request: Request) {
  ...
  try {
    ...
    const redirectTo = user.onboardingComplete ? '/candidate/dashboard' : '/candidate/dashboard';
    return NextResponse.redirect(new URL(redirectTo, baseUrl));
  } catch (error: any) {
    const msg = error?.message || String(error);
    const cause = error?.cause?.message || error?.cause?.code || '';
    console.error('Google user OAuth callback error:', msg, cause || '');
    const detail = encodeURIComponent(`${msg}${cause ? ' | ' + cause : ''}`.slice(0, 200));
